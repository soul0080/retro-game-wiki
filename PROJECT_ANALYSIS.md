...
| 运营方式 | 个人长期运营，AI 驱动内容生产效率 |

### 1.3 长期愿景

建立「AI 驱动的中文最大经典游戏知识数据库」，最终形成：

```
AI 生产效率 → 数据库沉淀资产 → SEO 流量 → 广告收入 → 反哺内容
```
...
| 缓存 | Redis（后期） | 热门页 + AI 任务队列 |
| 部署 | Vercel + Cloudflare + Supabase | 免费起步，月成本 < $20 |
| 爬虫 | Python 独立目录 | 不混入网站代码 |
...
| AI | ai_generations | AI 生成历史（prompt/model/output） |
...
| Phase 2 | 内容系统 | AI 生成 + 批量导入 + SEO + Sitemap |
| Phase 3 | 自动化 | 爬虫 + AI 审核 + 自动更新 |
...
| 5 | 后台 CMS | /admin + CRUD |
| 6 | AI 内容生产 | /ai + services/ai.ts + prompts |
| 7 | 爬虫系统 | crawler/ Python |
...
- 文档详细描述了流程，但未指定：
  - 用哪家 AI API（OpenAI / Claude / 通义 / 智谱）
  - 用什么模型、token 预算
  - 重试与失败策略
  - 成本控制
- `ai_generations` 表无 `cost` / `tokens` 字段

#### 问题 L：图片来源与版权未明确

- 用 Supabase Storage 存图
- 但未说明图片来源：手动上传？爬虫抓取？AI 生成？官方授权？
...
| 5 | **Phase 编号统一** | project-spec 4 阶段 / roadmap 11 阶段 | 以 roadmap 11 阶段为执行单元，归入 project-spec 4 大阶段 |

### 6.2 技术选型确认（P1，影响架构）

| # | 决策项 | 待确认 |
|---|--------|--------|
| 6 | **AI API 供应商** | OpenAI / Anthropic / 通义 / 智谱？模型与预算？ |
| 7 | **后台登录方案** | Supabase Auth / 环境变量密码 / NextAuth？ |
| 8 | **中文搜索方案** | ILIKE + 关键词字段 / Meilisearch / 其他？ |
| 9 | **Migration 工具** | Supabase CLI / 手动 SQL？命名规范？ |
| 10 | **页面渲染策略** | 游戏详情 ISR？列表 SSR？后台 CSR？revalidate 触发方式？ |
...
| # | 决策项 | 待确认 |
|---|--------|--------|
| 14 | **图片来源与版权策略** | 手动上传 / 授权 / AI 生成？版权规避方案？ |
| 15 | **media 表是否增加版权字段** | license / source / credit |
| 16 | **内容审核人** | 谁负责 AI 生成内容的人工审核？ |
...
- **AI 规范严格**：明确 draft → 审核 → published，避免垃圾内容
